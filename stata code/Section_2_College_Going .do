...
*******************************************************************************/
	...
	*version 17.0
	...
	ssc install carryforward //user-written package needed for analysis
	...
	******************************************/	
	...
	///Diagnostic Chart 2.a.1: Percentage of Graduating Class Entering College 1-5 Years from High School
	{
			...
			*1. Data set up
				...
				*generating the percent of students in each category
					...
					replace pct_enrolled_base1=(everenrolled_1/counter)*100 if years_from_hs==1
					
					gen pct_enrolled_base=0 //these are the bars for years 2-5
					replace pct_enrolled_base=(everenrolled_1/counter)*100 if years_from_hs==2
					replace pct_enrolled_base=(everenrolled_2/counter)*100 if years_from_hs==3
					replace pct_enrolled_base=(everenrolled_3/counter)*100 if years_from_hs==4
					replace pct_enrolled_base=(everenrolled_4/counter)*100 if years_from_hs==5
				...
				*this allows us to calculate the difference in percent ever enrolled year to year
					gen pct_enrolled_additional=0
						forvalues i=1/5 {
							replace pct_enrolled_additional=(everenrolled_`i'/counter)*100 if years_from_hs==`i'
						}
				...
				*calculate difference in percent ever enrolled year to year
					...
					keep pct_enrolled_yr_over_yr years_from_hs
				
				*adding row for sum of enrollment
					set obs `=_N+1'
					replace years_from_hs=6 in `=_N' //can change the 6 based on your time horizon. If you are measure enrollment in years 1-7 out of HS you could change this to an 8
				...
				*generating groups to categorize positive and negative flows year to year, you should not need to make changes here
					cap drop group
					gen group=.
					replace group=1 if years_from_hs==1 | id==`=_N'
	...
	//Diagnostic Chart 2.b.2: Percentage Enrolled in College 1 Year after HS by Demographic Charactaristics and Test Score
	{
				...
				preserve	
					...
					*Graph (remove [fw=counter2] if you do not wish to weight the markers)
					...
					if inlist("`val'", "RACE", "CATEGORICAL VARS WITH 4 CATEGORIES HERE" ){ //to add more categories, add another scatter statement and modify the `var' number, as well as the legend statement
							...
							(scatter pct_`var' `var2' if `var'==4 [fw=counter2], c(l) lp(shortdash) ms(T) mc(vermillion) lc(vermillion) msize(counter2)), ///
							title("Percentage Enrolled in College 1 Year after HS" ///
							`"by `vtext' and `vtext2'"') ///
							ytitle("Percentage Enrolled") ///
							ysc(r(0(10)100)) ylabel(0(10)100) ymtick(0(10)100) ///
							legend(order(1 "`:label (`var') 1'" 2 "`:label (`var') 2'" ///
							3 "`:label (`var') 3'" 4 "`:label (`var') 4'")) ///
