...
*******************************************************************************/
	...
	***************************************/
	...
	use "${path_to_data}" //for .dta files
	...
	******************************************/	
	...
	//Diagnostic Chart 1.a.1: Percentage of Graduating Classes with Postsecondary Credentials
	{
			*1. Data set up
			preserve
				...
				*generating new variables representing percent in each credential category
					...
					gen aa_pct=(pct_credential)*100 if HIGHEST_DEGREE_IN_YEAR==2 | HIGHEST_DEGREE_IN_YEAR==3 //sub-baccalaureate credential
			...
			*2. Graph
				graph bar aa_pct-grad_pct if inrange(years_from_hs,1,10), over(years_from_hs) stack ///
				...
				legend(order(3 2 1) label(1 "Associate, Certificate," "or Diploma") label(2 "Bachelors") ///
				label(3 "Graduate")) note("SAMPLE INFO HERE", size(small))
	...
	{
			foreach var in SEX RACE ENTER_OTHER_VARS_HERE {
				...
				*1. Data set up
					...
					*generating percent in each category
						...
						gen aa_pct=(counter/`var'_count)*100 if HIGHEST_DEGREE_IN_YEAR==2 | HIGHEST_DEGREE_IN_YEAR==3 //sub-baccalaureate credential
				...
				*2. Graph
					graph bar aa_pct-grad_pct  , over(`var') stack  ///
					...
					legend(label(3 "Graduate") label(2 "Bachelors") ///
					label(1 "Associate," "Certificate," "or Diploma") order(3 2 1))  ///
			...
			*2.Graph 
				foreach var in SEX RACE ENTER_OTHER_VARS_HERE {
					...
					*Graph
					...
					if inlist("`val'", "INSERT VARS WITH 4 CATEGORIES" )	{
