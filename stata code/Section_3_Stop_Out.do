...
*******************************************************************************/
	...
	*version 17.0
	...
	global path_to_data	"$path_to_folder/data/section3.dta" // set path to your saved .csv data file matching spec document
	...
	* enrollment status indicators
		...
		label values DEGREE_COMPLETER STOPOUT CURRENTLY_ENROLLED OUTCOME_MEASURED yes_no
		
		label variable DEGREE_COMPLETER "Postsecondary Completer"
		label variable STOPOUT "Stopped Out, No Degree"
		label variable CURRENTLY_ENROLLED "Enrolled, No Degree"
		label variable OUTCOME_MEASURED "Outcome is Available for Measurement"
	
	* degree in HS
		label variable DEG_IN_HS "Earned Degree in HS"
	
	* postsecondary academic variables
		label variable CREDIT_HOURS_FIRST_TERM "Credit Hours Earned in the First Term"
		label variable GPA_FIRST_TERM "GPA in the First Term"
		label variable CREDIT_HOURS_CUMMULATIVE "Total Credit Hours Earned"
		label variable FIRST_TERM "First Term"
		label variable ATTEMPTED_DEGREE_FIRST_TERM "Attempted Degree in the First Term"
		label variable STOP_OUT_TERM "Stop Out Term"
		label variable ATTEMPTED_DEGREE_STOP_OUT "Attempted Degree at Stop Out"
		label variable EVER_REENROLLED "Ever Re-Enrolled"
		label variable REENROLL_SEMESTER "Re-Enrollment Term"
	...
	******************************************/			
			...
			foreach var in SEX RACE MATH_TEST_QUARTILE {
				...
				*2. Graph	
					//note: change the title for this graph if you are measuring enrollment status at a time other than 6 years from college entry
						graph bar pct_stopout_neg pct_CURRENTLY_ENROLLED pct_DEGREE_COMPLETER, over(`var') stack ///
							...
							yline(0, lcolor(gray)) ///
							legend (order(2 1 3) ///
							label(1 "Stopped Out, No Degree" ) label(2 "Enrolled, No Degree") ///
							label(3 "Postsecondary Completer")) ///
	...
	//Diagnostic Chart 3.a.2: Percentage of Postsecondary Attempters who Stopped Out by Demographic Charactaristics and Test Score
	{
				...
				*2.Graph
				//note: change the title for this graph if you are measuring enrollment status at a time other than 6 years from college entry. Delete the [fw=counter2] option if you do not want the markers to be weighted.
					if inlist("`val'", "SEX") {
	...
	//Diagnostic Chart 3.a.3: Percentage of Degree Attempters who Stopped Out by Number of Credits Earned in First Semester
	{
		preserve
			*1. Data Cleaning 
				...
				*keeping those who attempted a certain degree in the first term. Change this based on your desired pathway. 
				...
				*identifying those who stopped out within 6 years of postsecondary entry by subtracting the stop out term from a students' first term
						...
						replace stopout=1 if years_between<=6 //students who stopped out within 6 years of entry. The number here will also depend on how your data are structured (i.e. whether your data use calendar year or school year). You will change this if you are looking at stop out within another timeframe. For example, if you are looking at AA degrees and want to examine outcomes at 3 years from entry.
	...
	//Diagnostic Chart 3.a.3:  Percentage of Degree Attempters who Stopped Out by GPA Earned in First Semester 
	{
			...
			*1. Data Cleaning 
				...
				*identifying those who stopped out within 6 years of postsecondary entry by subtracting the stop out term from a students' first term
						...
						gen stopout=0 if STOP_OUT_TERM==. //these are students who never stopped out 
						replace stopout=1 if years_between<=6 & !mi(STOP_OUT_TERM) //students who stopped out within 6 years of entry. The number here will also depend on how your data are structured (i.e. whether your data use calendar year or school year). You will change this if you are looking at stop out within another timeframe. For example, if you are looking at AA degrees and want to examine outcomes at 3 years from entry.
						drop if stopout==. //dropping those whose stop out status is unclear (if applicable)
			...
			*2. Prep for graph
					...
					*generating total in each bin
						bys gpa_bin  : egen total=total(counter) 
	...
	//Diagnostic Chart 3.a.4: Percentage of Stop Outs by Number Credits Away from Degree Minimums
	{
			...
			*3.Graph	
						...
						title ("Percentage of Stop Outs by Number Credits Away from Degree Minimum") ///
						 ysc(r(0(10)100)) ylabel(0(10)100) ymtick(0(10)100) ///
